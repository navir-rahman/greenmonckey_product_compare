const slideshow__button = (th, event, id) => {
  const targettoltip = document.getElementById("slideshow__tooltip" + id);
  const lowopacity = document.getElementsByClassName("opacity" + id);
  if (targettoltip.style.display === "none") {
    targettoltip.style.display = "block";
    lowopacity.forEach((s) => {
      s.style.opacity = 0.2;
    });
  } else {
    targettoltip.style.display = "none";
    lowopacity.forEach((s) => {
      s.style.opacity = 1;
    });
  }
...
(() => {
  ...
  var CustomHTMLElement = class extends HTMLElement {
    ...
    untilVisible(
      intersectionObserverOptions = { rootMargin: "0px", threshold: 0.2 }
    ) {
...
`;
  ...
  var FlickityCarousel = class extends CustomHTMLElement {
    ...
    async _onStaticClick(event, pointer, cellElement) {
      ...
      if (pointer.clientX > halfEdge) {
        ...
        if (parentel.classList.contains("sliderImageContainer")) {
          // alert(" It Worked!!! ");
          flickityInstance.next();
        }
        console.log("this", this, "/n parent", parentel);
      } else {
  ...
  var ShopTheLookNav = class extends CustomHTMLElement {
    ...
    transitionToIndex(index, shouldAnimate = true) {
      const indexElements = Array.from(
        ...
        currentElement = indexElements.find(
          (item) => !item.hasAttribute("hidden")
        ),
  ...
  var Timeline = class extends HTMLElement {
    ...
    get selectedIndex() {
      return this._selectedIndex; // this.listItemElements.findIndex((item) => !item.hasAttribute("hidden"));
    }
    previous() {
      this.select(Math.max(0, this._selectedIndex - 1));
    }
    next() {
      this.select(
        Math.min(this._selectedIndex + 1, this.listItemElements.length - 1)
      );
    }
    select(index, animate = true) {
  ...
  var ProductFacet = class extends CustomHTMLElement {
    ...
    async _rerender(event) {
      ...
      try {
        ...
        // rerenders product grid
        if (window.dontFilter == false) {
  ...
  const filterChangedEvent = new Event("filterChanged");
  var FacetFilters = class extends DrawerContent {
    ...
    _onFiltersCleared(event, target) {
      ...
      if ((window.isFilterCollection = true)) {
    ...
    _onFilterChanged() {
      ...
      //adds check for dontFilter variable set in facet-filters-horizontal.liquid
      triggerEvent(this, "facet:criteria-changed", {
        url: `${window.location.pathname}?${searchParamsAsString}`,
      });
    }
    _adjustDrawer(match) {
      if (!this.classList.contains("product-facet__filters--horizontal")) {
